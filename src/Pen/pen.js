...
export class Pen extends CanvasLayer {
    ...
    async analyze(data){
        ...
        if (command == 'penIdle') {         //no buttons pressed
            ...
            if (coords.x != undefined && coords.y != undefined) {
                ...
                else if (this.drawing != undefined && this.drawing.data.type == CONST.DRAWING_TYPES.POLYGON && this.newDrawing) {
                    const event = {
                        data: {
                            destination: {
                                x: scaledCoords.x, 
        ...
        if (command == 'penRight') {
            if (status == 'click') {
                if (this.menu.selectedDrawingTool == 1) {
                    ...
                    else if (tool == 'freehand') type = CONST.DRAWING_TYPES.FREEHAND;

                    drawingData.type = type;
                    drawingData.fillColor = this.menu.colors[this.menu.selectedFillColor].hex;
                    drawingData.fillType = this.menu.colors[this.menu.selectedFillColor].hex == 'none' ? 0 : 1;
                    drawingData.strokeColor = this.menu.colors[this.menu.selectedLineColor].hex;
                    drawingData.strokeAlpha = this.menu.colors[this.menu.selectedLineColor].hex == 'none' ? 0 : 1;
                ...
                if ((this.drawing.data.type == CONST.DRAWING_TYPES.POLYGON || this.drawing.data.type == CONST.DRAWING_TYPES.FREEHAND) && this.newDrawing) {
                ...
                if (this.drawing.data.type != CONST.DRAWING_TYPES.POLYGON) {
                    if (this.newDrawing) {
                        this.newDrawing = false;
                        canvas.drawings.preview.removeChild(this.drawing);
                        const cls = getDocumentClass('Drawing');
                        this.lastDrawing = await cls.create(this.drawing.data.toObject(false), {parent: canvas.scene});
                        this.drawing == undefined;
                    }
                    else {   
                        this.drawing.update({width:this.drawing.data.width,height:this.drawing.data.height});
                        this.lastDrawing = this.drawing;
                        this.drawing == undefined;
                    }
                }
            ...
            if (status == 'click') {
                if (this.newDrawing && this.drawing.data.type == CONST.DRAWING_TYPES.POLYGON) {
                    this.newDrawing = false;
                    canvas.drawings.preview.removeChild(this.drawing);
                    const cls = getDocumentClass('Drawing');
                    this.lastDrawing = await cls.create(this.drawing.data.toObject(false), {parent: canvas.scene});
                ...
                if (this.drawing == undefined || this.menu.selectedDrawing == 6) return;
                this.drawing.update({x:this.drawing.data.x,y:this.drawing.data.y})
            }
        }
        else if (command == 'penRear') {
            if (status == 'click') {
        ...
        if (command == 'penRight') {
            if (status == 'click') {
                ...
                if (this.newTemplate) {
                    ...
                    return cls.create(this.template.data.toObject(false), {parent: canvas.scene});
                }
                else    
                    this.template.update({distance:this.template.data.distance,direction:this.template.data.direction})
            }   
        }
        else if (command == 'penLeft') {
            if (status == 'click')
                ...
                if (this.template == undefined || this.menu.selectedTemplate == 6) return;
                const snappedPosition = canvas.grid.getSnappedPosition(data.x, data.y, canvas.templates.gridPrecision);
                this.template.update({x:snappedPosition.x,y:snappedPosition.y})
            }
        }
        else if (command == 'penRear') {
            if (status == 'click') {
                ...
                if (this.template == undefined || this.menu.selectedTemplate == 6) return;
                const snappedPosition = canvas.grid.getSnappedPosition(data.x, data.y, canvas.templates.gridPrecision);
                this.template.update({x:snappedPosition.x,y:snappedPosition.y,direction:this.template.data.direction})
            }  
        }
    }
    ...
    getNearestDrawing(data) {
        ...
        for (let drawing of drawings) {
            const dist = Math.abs(drawing.x+drawing.data.width/2-data.x) + Math.abs(drawing.y+drawing.data.height/2-data.y)
            if (dist < canvas.dimensions.size*5 && dist < nearestDistance) {
